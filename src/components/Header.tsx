    ...
    <header className="border-b border-line bg-ivory">
      <div className="mx-auto flex max-w-[1200px] items-center justify-between gap-6 px-6 py-4">
        ...
        <details className="lg:hidden relative">
          <summary className="list-none cursor-pointer text-sm text-ink border border-line rounded px-3 py-1.5">
            Menu
          </summary>
          <nav className="absolute right-0 z-20 mt-2 flex w-56 flex-col gap-1 rounded border border-line bg-ivory p-3 shadow-md">
            {nav.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="rounded px-2 py-1.5 text-sm text-charcoal hover:bg-paper"
              >
                {item.label}
              </Link>
            ))}
